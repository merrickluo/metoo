...
src_prepare() {
	...
	sed -i 's,Name=微信,Name=Wechat,' \
		"${S}/usr/share/applications/weixin.desktop" || die
	sed -i 's,Categories=Utility,Categories=Network,' \
		"${S}/usr/share/applications/weixin.desktop" || die
	sed -i 's,/opt/apps/com.tencent.weixin/files/weixin/weixin.sh,/usr/bin/weixin-bin,' \
		"${S}/usr/share/applications/weixin.desktop" || die
	sed -i 's,/opt/apps/com.tencent.weixin/files/weixin/weixin,/opt/weixin/weixin "$@",g' \
		"${S}/opt/apps/com.tencent.weixin/files/weixin/weixin.sh" || die

	# fix rpath
	patchelf --set-rpath /opt/weixin/resources/app/packages/main/dist/bin/scrot\
		"${S}/opt/apps/com.tencent.weixin/files/weixin/resources/app/packages/main/dist/bin/scrot/scrot" || die
}

src_install() {
	newmenu "${S}/usr/share/applications/weixin.desktop" weixin.desktop
	dobin "${FILESDIR}/weixin-bin"
	...
	fperms +x /opt/weixin/weixin{,.sh}
	fperms +x /opt/weixin/resources/app/packages/main/dist/bin/scrot/scrot

	insinto /opt/weixin/crap
	doins "${FILESDIR}"/uos-{lsb,release}

	insinto /opt/weixin/crap/usr/lib/license
	doins "${S}/usr/lib/license/libuosdevicea.so"
	keepdir /usr/lib/license

	insinto /opt/weixin/crap/var/uos
	newins "${FILESDIR}/license.key" .license.key

	insinto /opt/weixin/crap/var/lib/uos-license
